    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const buscar_cliente = tool({
          description: "Busca cliente do vendedor por nome ou documento (CPF/CNPJ). Use antes de pedir dados de cadastro.",
          inputSchema: z.object({ query: z.string().min(2) }),
          execute: async ({ query }) => {
            const digits = onlyDigits(query);
            let q = supabase.from("clients").select("id,name,cpf,cnpj,email,phone,cidade,uf").limit(5);
        ...
        const consultar_cep = tool({
          description: "Consulta endereço pelo CEP via ViaCEP. Use sempre que o vendedor informar um CEP novo.",
        ...
        const upsert_cliente = tool({
          ...
          execute: async (input) => {
            ...
            if (cpf) {
              const { data: r } = await supabase.from("clients").select("id").eq("cpf", cpf).maybeSingle();
              if (r) existingId = r.id;
            }
            if (!existingId && cnpj) {
              const { data: r } = await supabase.from("clients").select("id").eq("cnpj", cnpj).maybeSingle();
              if (r) existingId = r.id;
            }
            const payload = {
              user_id: userId,
        ...
        const criar_contrato = tool({
          description: "Cria o contrato e monta o snapshot do vendedor. Use depois de confirmar o resumo com o usuário.",
          inputSchema: z.object({
        ...
        const gerar_pdf_contrato = tool({
          description: "Gera o PDF do contrato e retorna uma URL temporária (10 min) para download.",
          inputSchema: z.object({
        ...
        async function contractInsertCore(input: {
          ...
          const { data: profile } = await supabase.from("profiles")
            .select("accepted_terms_version,company_legal_name,company_fantasy_name,company_cnpj,company_address,company_city,company_uf,company_cep,representative_name,representative_qualification,comarca")
            .eq("id", userId).maybeSingle();
          if (profile?.accepted_terms_version !== TERMS_VERSION) return { error: "Aceite os novos termos para continuar." };
          ...
          const { data: cli } = await supabase.from("clients").select("id,name").eq("id", input.client_id).maybeSingle();
        ...
        async function pdfCore(contract_id: string, parcelas: number | null) {
          ...
          const { data: prof } = await supabase.from("profiles").select("logo_path").eq("id", userId).maybeSingle();
          ...
          const { error: upErr } = await supabaseAdmin.storage.from("contract-pdfs").upload(path, pdfBytes, {
        ...
        return result.toUIMessageStreamResponse({
          ...
          onFinish: async ({ messages }) => {
            ...
            const { data: existing } = await supabase
              .from("chat_threads").select("id").eq("contract_id", body.contractId).maybeSingle();
            const payload = { messages: messages as never };
            if (existing) {
              await supabase.from("chat_threads").update(payload).eq("id", existing.id);
            } else {
              await supabase.from("chat_threads").insert({
                contract_id: body.contractId, user_id: userId, ...payload,
              } as never);
            }
          },
        });
